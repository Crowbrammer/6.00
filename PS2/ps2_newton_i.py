	...
	try: 
		...
		for a in range(len(poly)):
			...
			try: 
				...
				f +=  (poly[a]*x**a,)
				print('f12: ' + str(f))
				print(str(checker(f)) + ' at Line 156')
				...
				print('f18: ' + str(f))
				print(str(checker(f)) + ' at Line 161')
		
	except TypeError:
	
		f = (poly,)
		print('f24: ' + str(f))
		print(str(checker(f)) + ' at Line 166')
	
	print('f27: ' + str(f))
	print(str(checker(f)) + ' at Line 169') 
...
def checker(checkee):
	...
	return all(isinstance(x, float) for x in checkee)
	

poly = (-13.39, 0.0, 17.5, 3.0, 1.0)
x = 6
print('checker(evaluate_poly(poly, x)): ' + str(checker(evaluate_poly(poly, x))))
